...
return {
  ...
  },
  {
    ...
    run = function()
      ...
      H.same({ 'w3m', '-T', 'text/html', '-dump', '$path' }, find_rule(view_rules, 'html').commands[1])
      H.same({ 'lynx', '-dump', '-nolist', '$path' }, find_rule(view_rules, 'html').commands[2])
      H.same({ 'elinks', '-dump', '-no-references', '$path' }, find_rule(view_rules, 'html').commands[3])
  ...
  },
  {
    ...
    run = function()
      ...
      H.eq(true, rules.matches(find_rule(view_rules, 'html'), att({ part = { content_type = 'text/html', filename = 'body.html', ext = 'html' } })))
      H.eq(true, rules.matches(find_rule(view_rules, 'pdf'), att({ part = { content_type = 'application/pdf', filename = 'doc.pdf', ext = 'pdf' } })))
      H.eq(true, rules.matches(find_rule(view_rules, 'pdf'), att({ part = { content_type = 'application/octet-stream', filename = 'doc.pdf', ext = 'pdf' } })))
      H.eq(true, rules.matches(find_rule(view_rules, 'image'), att({ part = { content_type = 'image/png', filename = 'img.png', ext = 'png' } })))
      H.eq(true, rules.matches(find_rule(view_rules, 'office'), att({ part = { content_type = 'application/octet-stream', filename = 'doc.docx', ext = 'docx' } })))
      H.eq(true, rules.matches(find_rule(view_rules, 'markdown'), att({ part = { content_type = 'text/markdown', filename = 'README.md', ext = 'md' } })))
      H.eq(true, rules.matches(find_rule(view_rules, 'zip'), att({ part = { content_type = 'application/zip', filename = 'archive.zip', ext = 'zip' } })))
      H.eq(true, rules.matches(find_rule(view_rules, 'tar'), att({ part = { content_type = 'application/gzip', filename = 'archive.tar.gz', ext = 'gz' } })))
      H.eq(true, rules.matches(find_rule(view_rules, 'text'), att({ part = { content_type = 'text/plain', filename = 'note.txt', ext = 'txt' } })))
      H.eq(true, rules.matches(find_rule(view_rules, 'binary'), att({ part = { content_type = 'application/octet-stream', filename = 'blob.bin', ext = 'bin' } })))
    end,
  },
  {
